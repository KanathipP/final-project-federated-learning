...
def pipeline(partition_id: int):
    ...
    groups.append([partition_id]*len(labels))    
    groups = np.concatenate(groups)
    features = np.moveaxis(features, 1, 2)  # (N, T, C) หลังจากบรรทัดนี้

    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    train_index, val_index = next(sss.split(features, labels))
    ...
    train_features = np.moveaxis(train_features, 1, 2)  # กลับไป (N, C, T)
